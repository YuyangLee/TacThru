...
def concatenate_normalizer(normalizers: list):
    ...
    input_stats_dict = dict_apply_reduce(
        [normalizer.params_dict["input_stats"] for normalizer in normalizers], lambda x: torch.concatenate(x, axis=-1)
    )
    return SingleFieldLinearNormalizer.create_manual(scale=scale, offset=offset, input_stats_dict=input_stats_dict)
