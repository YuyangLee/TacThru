...
class FisheyeRectConverter:
    def __init__(self, K, D, DIM, out_size, out_fov, principal_point_offset=(0, 0)):
        out_size = np.array(out_size)
        # vertical fov
        out_f = (out_size[1] / 2) / np.tan(out_fov / 180 * np.pi / 2)
        cx = out_size[0] / 2 + principal_point_offset[0]
        cy = out_size[1] / 2 + principal_point_offset[1]
        # out_K = np.array([[out_f, 0, out_size[0] / 2], [0, out_f, out_size[1] / 2], [0, 0, 1]], dtype=np.float32)
        out_K = np.array([[out_f, 0, cx], [0, out_f, cy], [0, 0, 1]], dtype=np.float32)
    ...
    def forward(self, img):
        rect_img = cv2.remap(img, self.map1, self.map2, interpolation=cv2.INTER_AREA, borderMode=cv2.BORDER_CONSTANT)
        return rect_img
...
def detect_localize_aruco_tags(
    ...
    for this_id, this_corners in zip(ids, corners):
        ...
        tag_dict[this_id] = {"rvec": rvec.squeeze(), "tvec": tvec.squeeze(), "corners": this_corners.squeeze()}
    return tag_dict
...
def get_charuco_board(
    ...
    board = cv2.aruco.CharucoBoard(
        size=grid_size, squareLength=square_length_mm / 1000, markerLength=tag_length_mm / 1000, dictionary=aruco_dict
    )
    return board
...
def get_gripper_width(tag_dict, left_id, right_id, nominal_z=0.072, z_tolerance=0.02):
    zmax = nominal_z + z_tolerance
    zmin = nominal_z - z_tolerance
    left_x = None
    left_tvec = None
    if left_id in tag_dict:
        left_tvec = tag_dict[left_id]["tvec"]
        if zmin < left_tvec[-1] < zmax:
            left_x = left_tvec[0]
    right_x = None
    right_tvec = None
    if right_id in tag_dict:
        right_tvec = tag_dict[right_id]["tvec"]
        if zmin < right_tvec[-1] < zmax:
            right_x = right_tvec[0]
    if left_tvec is not None and not (zmin < left_tvec[-1] < zmax):
        print(f"Tag {left_id} depth {left_tvec[-1]} out of range ({zmin}, {zmax})")
    if right_tvec is not None and not (zmin < right_tvec[-1] < zmax):
        print(f"Tag {right_id} depth {right_tvec[-1]} out of range ({zmin}, {zmax})")
    width = None
...
def get_mirror_canonical_polygon():
    left_pts = [[540, 1700], [680, 1450], [590, 1070], [290, 1130], [290, 1770], [550, 1770]]
...
def get_gripper_canonical_polygon():
    left_pts = [[1352, 1730], [1100, 1700], [650, 1500], [0, 1350], [0, 2028], [1352, 2704]]
    resolution = [2028, 2704]
    left_coords = pixel_coords_to_canonical(left_pts, resolution)
    right_coords = left_coords.copy()
    right_coords[:, 0] *= -1
...
def get_finger_canonical_polygon(height=0.37, top_width=0.25, bottom_width=1.4):
    ...
    points = [[[bottom_left_x, bottom_y], [top_left_x, top_y], [top_right_x, top_y], [bottom_right_x, bottom_y]]]
    coords = pixel_coords_to_canonical(points, img_shape=resolution)
    return coords


@cache
def get_mask_polygon_coords(mirror=True, gripper=True, finger=True):
    all_coords = list()
    if mirror:
...
def draw_predefined_mask(img, color=(0, 0, 0), mirror=True, gripper=True, finger=True, use_aa=False):
    all_coords = get_mask_polygon_coords(mirror=mirror, gripper=gripper, finger=finger)
    for coords in all_coords:
        pts = canonical_to_pixel_coords(coords, img.shape[:2])
        pts = np.round(pts).astype(np.int32)
        flag = cv2.LINE_AA if use_aa else cv2.LINE_8
        # if different size, resize pts to match img shape

        img = cv2.fillPoly(img.copy(), [pts], color=color, lineType=flag)
    return img


def draw_predefined_mask_720p(img, color=(0, 0, 0), mirror=True, gripper=True, finger=True, use_aa=False):
    all_coords = get_mask_polygon_coords(mirror=mirror, gripper=gripper, finger=finger)
    for coords in all_coords:
        pts = canonical_to_pixel_coords(coords, img.shape[:2])
        pts = np.round(pts).astype(np.int32)
...
def inpaint_tag(img, corners, tag_scale=1.4, n_samples=16):
    ...
    sample_colors = img[
        np.clip(sample_points[:, 1], 0, img.shape[0] - 1), np.clip(sample_points[:, 0], 0, img.shape[1] - 1)
    ]
