...
from diffusion_policy.common.cv_util import FisheyeRectConverter, parse_fisheye_intrinsics
...
def get_gopro_undisrotion_transform(out_fov=120, out_size=(224, 224), bgr_to_rgb=False):
    json_data = json.load(open("cfg/gopro/gopro_intrinsics_720p.json"))
    opencv_intr_dict = parse_fisheye_intrinsics(json_data)
    fisheye_converter = FisheyeRectConverter(
        K=opencv_intr_dict["K"],
        D=opencv_intr_dict["D"],
        DIM=opencv_intr_dict["DIM"],
        out_size=out_size,
        out_fov=out_fov,
    )
    if bgr_to_rgb:
