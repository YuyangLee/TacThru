...
# @torch.jit.script
def normalize_torch(vec, eps: float):
...
def rot6d_to_mat_torch(d6):
    a1, a2 = d6[..., :3], d6[..., 3:]
    b1 = normalize_torch(a1, 1e-12)
    b2 = a2 - torch.sum(b1 * a2, dim=-1, keepdims=True) * b1
    b2 = normalize_torch(b2, 1e-12)
...
# @torch.jit.script
def pose9d_to_mat_torch(d10):
    pos = d10[..., :3]
    d6 = d10[..., 3:]
    rotmat = rot6d_to_mat_torch(d6)
    out = torch.zeros(d10.shape[:-1] + (4, 4), dtype=torch.float32, device=d10.device)
    out[..., :3, :3] = rotmat
    out[..., :3, 3] = pos
    out[..., 3, 3] = 1
    return out
