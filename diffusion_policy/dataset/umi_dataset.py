...
class UmiDataset(BaseDataset):
    def __init__(
                    ...
                    try:
                        with zarr.LMDBStore(
                            ...
                            with zarr.ZipStore(dataset_path, mode="r") as zip_store:
                                ...
                                ReplayBuffer.copy_from_store(src_store=zip_store, store=lmdb_store)
        ...
        self.tacthru_keys = list()
        for key, attr in obs_shape_meta.items():
            if key not in replay_buffer.keys() and key != "robot0_eef_rot_axis_angle_wrt_start":
                print(f"Warning: skip key {key} because it is missing in dataset")
                continue
            # solve obs type
            ...
            elif key_type == "tac_rgb":
                self.tacthru_keys.append(key)
            elif key_type == "tac_sparse_shear":
                self.tacthru_keys.append(key)
            elif key_type == "tac_shear":
                self.tacthru_keys.append(key)
            elif key_type == "tac_normal":
                self.tacthru_keys.append(key)
            elif key_type == "tac_prox":
                self.tacthru_keys.append(key)
        ...
        self.sampler = SequenceSampler(
            ...
            tacthru_keys=self.tacthru_keys,
        ...
        self.proprio_stretching = proprio_stretching

    @property
    def all_keys(self) -> list[str]:
        return self.rgb_keys + self.tacthru_keys + self.lowdim_keys

    def get_open_loop_validation_dataset(self):
        ol_val_set = copy.copy(self)
        ol_val_set.sampler = SequenceSampler(
            ...
            curr_index_interval=self.key_horizon["action"],
        )
        ol_val_set.val_mask = ~self.val_mask
        return ol_val_set
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        action_normalizers.append(
            get_range_normalizer_from_stat(
                array_to_stats(data_cache["action"][..., robot_id * dim_a : robot_id * dim_a + 3])
            )
        )  # pos
        action_normalizers.append(
            get_identity_normalizer_from_stat(
                array_to_stats(data_cache["action"][..., robot_id * dim_a + 3 : (robot_id + 1) * dim_a - 1])
            )
        )  # rot
        action_normalizers.append(
            get_range_normalizer_from_stat(
                array_to_stats(data_cache["action"][..., (robot_id + 1) * dim_a - 1 : (robot_id + 1) * dim_a])
            )
        )  # gripper
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key in self.tacthru_keys:
            ...
            if obs_dict[key].ndim == 3:  # if (T, H, W)
                obs_dict[key] = obs_dict[key][:, np.newaxis, :, :]  # change into (T, 1, H, W)
        ...
        # proprioception velocity augmentation
        if eef_pos.shape[0] == 1 or not self.proprio_stretching:
            eef_pos_aug = eef_pos
            eef_rot_aa_aug = eef_rot_aa
        else:
            # randomly generate even samples in [alpha, 1], with alpha from 0 - 0.5
            # then do SE(3) interpolation
            t_ls = np.linspace(0, 1, eef_pos.shape[0])
            aug_t_ls = np.linspace(np.random.uniform(0.0, 0.5), 1, eef_pos.shape[0])
            eef_pos_aug = si.interp1d(
                t_ls,
                eef_pos,
                axis=0,
                bounds_error=False,
                fill_value=(eef_pos[0], eef_pos[-1]),
            )(aug_t_ls).astype(np.float32)
            eef_rot_aa_aug = Slerp(t_ls, R.from_rotvec(eef_rot_aa))(aug_t_ls).as_rotvec().astype(np.float32)
        (
            obs_dict[f"robot{robot_id}_eef_pos"],
            obs_dict[f"robot{robot_id}_eef_rot_axis_angle"],
        ) = (eef_pos_aug, eef_rot_aa_aug)
        # convert pose to mat
        pose_mat = pose_to_mat(np.concatenate([eef_pos_aug, eef_rot_aa_aug], axis=-1))

        # get start pose
        start_pose = obs_dict[f"robot{robot_id}_demo_start_pose"][0]
        # add noise to episode start pose
