...
def get_intersection_slice_mask(shape: tuple, dim_slices: Sequence[slice], device: torch.device | None = None):
...
def get_union_slice_mask(shape: tuple, dim_slices: Sequence[slice], device: torch.device | None = None):
    assert len(shape) == len(dim_slices)
    mask = torch.zeros(size=shape, dtype=torch.bool, device=device)
