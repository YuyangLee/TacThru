...
from diffusion_policy.model.diffusion.transformers_ext import TransformerDecoder, TransformerDecoderLayer
...
class TransformerForActionDiffusion(ModuleAttrMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        need_attn_weight = kwargs.get("need_attn_weight", False)
        ...
        tc = cond_emb.shape[1]
        cond_pos_emb = self.cond_pos_emb[:, :tc, :]  # each position maps to a (learnable) vector
        ...
        # 4. transformer
        # FIXME: Returning attention weights makes learning results bad!
        # x = self.decoder(tgt=input_emb, memory=cond_emb, need_attn_weight=need_attn_weight)
        x = self.decoder(tgt=input_emb, memory=cond_emb, need_attn_weight=need_attn_weight)
        if need_attn_weight:
            x, attn_weights = x
        x = self.ln_f(x)
        x = self.head(x)
        if need_attn_weight:
            return x, torch.stack(attn_weights, dim=-1)  # B x T x (N_tokens + 1) x N_layer
        return x
