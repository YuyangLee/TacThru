...
from diffusion_policy.model.vision.timm_obs_encoder import TimmTacThruObsEncoder
...
class DiffusionUnetTimmPolicy(BaseImagePolicy):
    def __init__(
        ...
        obs_encoder: TimmTacThruObsEncoder,
    ...
    def compute_loss(self, batch):
        ...
        if self.train_diffusion_n_samples != 1:
            ...
            global_cond = torch.repeat_interleave(global_cond, repeats=self.train_diffusion_n_samples, dim=0)
            nactions = torch.repeat_interleave(nactions, repeats=self.train_diffusion_n_samples, dim=0)
