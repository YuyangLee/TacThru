...
from diffusion_policy.model.vision.transformer_obs_encoder import TransformerTacThruObsEncoder
...
class DiffusionTransformerTimmPolicy(BaseImagePolicy):
    def __init__(
        ...
        obs_encoder: TransformerTacThruObsEncoder,
        ...
        **kwargs,
        ...
        obs_tokens = obs_encoder.example_output()

        obs_shape = obs_tokens.shape
        assert obs_shape[-1] == n_emb
        num_obs_tokens = obs_shape[-2]
        ...
        model = TransformerForActionDiffusion(
            ...
            n_head=n_head,
            n_emb=n_emb,
            max_cond_tokens=num_obs_tokens + 1,  # obs tokens + 1 token for time
        ...
        # self.token_layernorm = torch.nn.LayerNorm(n_emb)

    def _obs_dict_to_tokens(self, obs_dict: Dict[str, torch.Tensor]) -> tuple[torch.Tensor, list[str]]:
        tokens, token_ids = [], []
        for token_id in sorted(obs_dict.keys()):
            tokens.append(obs_dict[token_id])
            token_ids += [token_id] * obs_dict[token_id].shape[1]
        tokens = torch.cat(tokens, dim=1)
        return tokens, token_ids + ["noise_t"]
    ...
    def conditional_sample(
        ...
        **kwargs,
        ...
        for t in scheduler.timesteps:
            ...
            model_output = model(trajectory, t, cond, need_attn_weight=need_attn_weight)
            if need_attn_weight:
                model_output, attn_weights = model_output
    ...
    def predict_action(
        ...
        # process input
        obs_dict = self.obs_encoder(nobs, as_dict=True, no_randomize=no_randomize)
        obs_tokens, token_ids = self._obs_dict_to_tokens(obs_dict)

        # obs_tokens = self.token_layernorm(obs_tokens)
        ...
        nsample = self.conditional_sample(
            ...
            cond=obs_tokens,
            need_attn_weight=need_attn_weight,
            **self.kwargs,
        )
        if need_attn_weight:
            nsample, attn_weights = nsample
        ...
        if need_attn_weight:
            ...
            result.update(attn_weight_dict)
        return result
    ...
    def compute_loss(self, batch):
        ...
        # process input
        obs_tokens = self.obs_encoder(nobs, as_dict=False)
        # obs_tokens = self.token_layernorm(obs_tokens)
        # (B, N, n_emb)
